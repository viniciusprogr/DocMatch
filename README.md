...
---
📖 Visão Geral
O Smart Health Recommendation não é apenas um buscador de médicos, mas uma plataforma completa de gestão da jornada de saúde. O projeto evolui em camadas: começando com um assistente conversacional que recomenda profissionais baseados em geolocalização e regras de ranking, até chegar à gestão de medicamentos, histórico clínico, cuidadores e serviços de Home Care.
...
[ ] Sprint 01: Chatbot MVP (P0) - Busca por linguagem natural, extração de entidades e ranking de profissionais via Google Places.
...
Backend é Autoridade: A IA nunca é a fonte definitiva da verdade dos dados. O cálculo de score (Score = Distância × 0,40 + Avaliação × 0,30 + Relevância × 0,20 + Especialidade × 0,10) é exclusividade do sistema.
Sem Decisão Clínica: A IA NÃO realiza diagnósticos, prescrições, triagem clínica ou interpretação de exames.
Privacidade e Consentimento: Os dados de saúde são estritamente controlados. O acesso por terceiros (Familiares/Cuidadores) exige consentimento explícito e granular por tipo de recurso.
Situações de Emergência: Em risco de vida, o sistema interrompe o fluxo normal e apresenta orientação clara para busca imediata de um serviço de emergência.
Auditoria (Compliance): Ações críticas (alterar medicações, permissões, conclusão de consultas) geram logs imutáveis.
---
💻 Como rodar o projeto localmente
(Instruções a serem preenchidas conforme o avanço do Sprint 01)
```bash
# Clone o repositório
git clone https://github.com/seu-usuario/smart-health-recommendation.git

# Acesse a pasta do projeto
cd smart-health-recommendation
...
```
---
Documentação baseada na "Especificação Técnica - Smart Health Recommendation".
